...
3. Columns: sort lines by x; walk sorted x-centers, new column when `x - prev_x > 0.08 * img_width`. Cells (AMENDED after Task-5 evidence — a card's internal artwork band produces a larger text y-gap than the inter-card gutter, so pure gap clustering splits cards): within each column sort by y; the collector number is the card's bottom-edge ANCHOR: each line joins the nearest `pattern_ok` number line at/above `line_y − 0.04 * img_height` (`_FOOTER_TOL=0.04` snaps a card's own footer — copyright/flavor lines just below its number — back to its card; the literal "break after the number line" leaked footers into the next cell and produced a confident-wrong). Columns with no readable number fall back to the `y - prev_y > 0.12 * img_height` gap rule. (AS SHIPPED in 35d7f7c after Task-5 evidence.) `cols = n_columns`, `rows = max(len(cells_in_col) for col)`.
...
- [ ] **Step 1:** `scripts/make_binder_fixture.py` — builds `tests/corpus/binder/synthetic_3x3.jpg`: downloads 9 TCGdex card images spanning ≥3 sets (hardcode this exact list of ids: sv06 126, sv06 101, me05 004, me05 010, me02.5 001, me02.5 003, sv03.5 025, swsh12 040, me01 020; URL pattern `https://assets.tcgdex.net/en/<series>/<set>/<lid3>/high.png` — derive series dir from the set id prefix, e.g. sv06→sv, me05→me, swsh12→swsh; skip cleanly with a message if offline), pastes them at REALISTIC card aspect — 600×838 (63:88) — in a 3×3 grid on a dark-gray 2400×3200 canvas with 60px gutters, saves JPEG q88. (AMENDED: the fixture must keep true card geometry so the artwork-gap failure mode stays exercised; the number-separator clustering rule is what handles it, never squat cards.) Also writes `tests/corpus/binder/truth.json` `{"cards": [{"set": ..., "local_id": ...} × 9]}`.
