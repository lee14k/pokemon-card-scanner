...
## Binder scan → Collection (NOT a training input)

Binder page scans (`POST /scan/binder` → `POST /collection`) build a trainer's
personal Collection (`collection_card`), which is a wishlist/inventory — **not**
pull history. Collection ≠ pulls: binder scans never feed the training-upload
harvest, `rederive_pending()`, or any pull/set stat. Nothing about a saved
Collection card flows back into `training/data/`, `eval_sets.json`, or the
matcher indexes. The scan pipeline reuses the live-identify ladder for read-out
only; the whole-page photo is discarded after cells are cropped (no
staircase/frame artifacts are persisted).

`tests/corpus/binder/` holds the synthetic gate fixture (`synthetic_3x3.jpg` +
`truth.json`, generated by `scripts/make_binder_fixture.py`) that stands in for
real binder photos until real ones land. When real binder photos arrive: drop
them into `tests/corpus/binder/`, re-run the Task-5 gate script
(`app.pack.binder.scan_binder_page`) against each, expected = every visible card
identified-or-flagged (correct set/number/name, or flagged `needs_review` —
never silently wrong), grid matching the page's real rows×cols.
